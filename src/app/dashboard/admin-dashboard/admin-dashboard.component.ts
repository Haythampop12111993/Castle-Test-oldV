...
export class AdminDashboardComponent implements OnInit {
  ...
  statsData: any = {
    salesPipelineStats: null,
    inventoryStats: null,
    activityStats: null,
    leadStatusStats: null,
    paymentCollectedPipelineStats: null,
    ticketsStatuses: null,
  };
  ...
  getReminders() {
    this.http.get(`${environment.api_base_url}get_coming_reminders`).subscribe(
      (data: any) => {
        this.raw_events = data;
        this.calendar_events = data.map((reminder) => ({
          id: reminder.id,
          ...
          classNames: reminder.is_done ? ["done"] : [],
        }));
        this.initCalender();
      },
      (err) => {
  ...
  onChangeFilter(
    ...
    this.getTarget(
      ...
      this.selectedTeam_filter ? this.selectedTeam_filter.id : ""
    );
  }
  ...
  filterDashboard() {
    ...
    this.filterSalesPipeline(params);
    this.filterInventoryPipeline(params);
    this.filterActivity(params);
    this.filterLeadStatus(params);
    this.filterPaymentCollectedPipeline(params);
    this.filterTiketsPipeline(params);
  }

  filterSalesPipeline(params) {
    this.http
      .get(`${environment.api_base_url}dashboard/salesPipelineStats`, {
        params,
      })
      .subscribe((res) => {
        this.statsData.salesPipelineStats = res;
      });
  }

  filterInventoryPipeline(params) {
    this.http
      .get(`${environment.api_base_url}dashboard/inventoryStats`, {
        params,
      })
      .subscribe((res) => {
        this.statsData.inventoryStats = res;
      });
  }

  filterActivity(params) {
    this.http
      .get(`${environment.api_base_url}dashboard/activityStats`, {
        params,
      })
      .subscribe((res) => {
        this.statsData.activityStats = res;
      });
  }

  filterLeadStatus(params) {
    this.http
      .get(`${environment.api_base_url}dashboard/leadStatusStats`, {
        params,
      })
      .subscribe((res) => {
        this.statsData.leadStatusStats = res;
      });
  }

  filterPaymentCollectedPipeline(params) {
    this.http
      .get(
        `${environment.api_base_url}dashboard/paymentCollectedPipelineStats`,
        {
          params,
        }
      )
      .subscribe((res) => {
        this.statsData.paymentCollectedPipelineStats = res;
        this.startPaymentCardsSlider("", true);
      });
  }
  filterTiketsPipeline(params) {
    this.http
      .get(`${environment.api_base_url}dashboard/ticketsStats`, {
        params,
      })
      .subscribe((res) => {
        this.statsData.ticketsStatuses = res;
      });
  }
  onGoPageTop() {
  ...
  onLeadStatusClicked(lead_status) {
    if (this.userData.role == "Development Director") return;
    let statuses = [
      { id: 1, name: "New" },
      ...
      if (stat.name == lead_status.status) {
        ...
        this.router.navigate(["/pages/leads"], {
          queryParams: {
            ...
            team_id: JSON.stringify(this.selectedTeam),
            user_id: JSON.stringify(this.selectedUser),
          },
        });
        return;
      }
  ...
  timeouts = {};
  // stop use oldType to stop and start all
  startPaymentCardsSlider(oldType, first = false) {
    Object.keys(this.viewPaymentCardsSlider).map((type) => {
      if (first || this.timeouts[type] === null) {
        this.viewPaymentCardsSlider[type].change = false;
        this.timeouts[type] = setInterval(() => {
          this.viewPaymentCardsSlider[type].change = true;
          setTimeout(() => {
            this.viewPaymentCardsSlider[type].value =
              (this.viewPaymentCardsSlider[type].value + 1) %
              this.paymentCardsSliderValues.length;
            this.viewPaymentCardsSlider[type].change = false;
          }, 500);
        }, 6000);
      }
  ...
  stopPaymentCardsSlider(oldType) {
    Object.keys(this.viewPaymentCardsSlider).map((type) => {
      if (this.timeouts[type]) {
        clearInterval(this.timeouts[type]);
