...
export class AccountantDashboardComponent implements OnInit {
  ...
  getPaymentCollections() {
    ...
    this.reservationService.getAllPaymentCollections().subscribe(
      (res: any) => {
        ...
        this.payment_collections = res.data;
      },
      (err) => {
    ...
    this.http
      ...
      .subscribe((res) => {
        ...
        Object.keys(this.viewPaymentCardsSlider).map((type) => {
          this.startPaymentCardsSlider(type, true);
        });
      });
  }
  ...
  // PaymentCardsSlider
  paymentCardsSliderValues = ["all", "collected", "rejected", "remaining"];
  timeouts = {};
  startPaymentCardsSlider(type, first = false) {
