...
Tafgeet.prototype.parse = function (prefixes = true) {
  var serialized = [];
  var tmp = [];
  var inc = 1;
  ...
  if (this.length() >= 1 && this.length() <= 3) {
    ...
    for (i = 0; i < serialized.length; i++) {
      ...
      if (column == null || column + 1 > this.columns.length) {
        ...
        if (str != "" && column == 3 && i == 1) {
          var lastChar = str.substr(str.length - 2);
          if (lastChar == " و") {
            str +=
              this.addSuffixPrefix(serialized[i], column) + concats[column];
          } else {
            str +=
              " و" +
              this.addSuffixPrefix(serialized[i], column) +
              concats[column];
          }
        } else {
